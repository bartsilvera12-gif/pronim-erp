...
BEGIN
  ...
  RAISE NOTICE '── Test 4: pronimerp.dashboard_views tiene slug=clientes ──';
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'pronimerp' AND table_name = 'dashboard_views'
  ) THEN
    RAISE NOTICE 'SKIP 4: pronimerp.dashboard_views no existe';
  ELSE
    SELECT COUNT(*) INTO v_count FROM pronimerp.dashboard_views WHERE slug = 'clientes';
    IF v_count <> 1 THEN
      RAISE EXCEPTION 'FAIL 4: pronimerp.dashboard_views.clientes no encontrado';
    END IF;
    ...
    IF NOT EXISTS (
      ...
      RAISE NOTICE 'SKIP 5: pronimerp.empresa_dashboard_views no existe';
    ELSE
      SELECT COUNT(*) INTO v_count
      FROM pronimerp.empresa_dashboard_views edv
      JOIN pronimerp.dashboard_views dv ON dv.id = edv.dashboard_view_id
      WHERE edv.empresa_id = v_empresa AND dv.slug = 'clientes' AND edv.activo = true;
      IF v_count <> 1 THEN
        RAISE NOTICE 'SKIP 5: vista clientes no habilitada aún (correr migración 20260823000001)';
      ELSE
