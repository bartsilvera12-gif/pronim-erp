/**
 * Migración: expandir origen_creacion + agregar origen_detalle.
 */
...
async function main() {
  const sql = readFileSync(
    join(process.cwd(), "supabase", "migrations", "20260326130000_crm_origen_creacion_expand.sql"),
    "utf-8"
  );
  const client = new Client({ connectionString: getDbUrl(), ssl: { rejectUnauthorized: false } });
  try {
    await client.connect();
    console.log("Ejecutando migración crm_origen_creacion_expand...");
