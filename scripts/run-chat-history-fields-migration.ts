/**
 * Migración: campos de auditoría de historial + normalización de teléfonos.
 */
...
async function main() {
  const sql = readFileSync(
    join(process.cwd(), "supabase", "migrations", "20250329150000_chat_history_audit_fields.sql"),
    "utf-8"
  );
  const client = new Client({ connectionString: getDbUrl(), ssl: { rejectUnauthorized: false } });
  try {
    await client.connect();
    console.log("Ejecutando migración historial omnicanal...");
