/**
 * Migración: captura de texto libre en flujo conversacional.
 */
...
async function main() {
  const sql = readFileSync(
    join(process.cwd(), "supabase", "migrations", "20250329180000_chat_flow_text_capture.sql"),
    "utf-8"
  );
  const client = new Client({ connectionString: getDbUrl(), ssl: { rejectUnauthorized: false } });
  try {
    await client.connect();
    console.log("Ejecutando migración text capture del flow...");
