/**
 * Migración: base para UI de administración de flujos.
 */
...
async function main() {
  const sql = readFileSync(
    join(process.cwd(), "supabase", "migrations", "20250329200000_chat_flow_admin_ui_base.sql"),
    "utf-8"
  );
  const client = new Client({ connectionString: getDbUrl(), ssl: { rejectUnauthorized: false } });
  try {
    await client.connect();
    console.log("Ejecutando migración base UI flujos...");
