/**
 * Migración: chat_messages + chat_conversations en publicación supabase_realtime
 */
...
async function main() {
  const sql = readFileSync(
    join(process.cwd(), "supabase", "migrations", "20250329140000_chat_realtime_publication.sql"),
    "utf-8"
  );
  const client = new Client({ connectionString: getDbUrl(), ssl: { rejectUnauthorized: false } });
  try {
    await client.connect();
    console.log("Ejecutando migración realtime publication (chat)...");
