...
`TEST_DB_URL` no está definida, si el nombre de la base no contiene `test` o
si no se confirma explícitamente `ALLOW_DESTRUCTIVE_TEST_DB=true`. Al iniciar,
el runner elimina y reconstruye el schema `pronimerp`.
