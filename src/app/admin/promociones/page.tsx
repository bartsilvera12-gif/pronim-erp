...
type ClientePicker = { id: string; nombre: string; empresa: string | null; ruc: string | null; telefono: string | null };
...
const TIPO_LABEL: Record<string, string> = {
  ...
  cashback: "Cashback %",
  cashback_fijo: "Cashback Gs.",
};

function fmtValor(p: Promo): string {
  if (p.tipo === "descuento_pct") return `${p.valor}%`;
  if (p.tipo === "cashback") return `${p.valor}% cashback`;
  if (p.tipo === "cashback_fijo") return `Gs. ${Math.round(Number(p.valor)).toLocaleString("es-PY")} cashback`;
...
function NuevaPromocionModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const [tipo, setTipo] = useState<"descuento_pct" | "descuento_fijo" | "lleve_n_pague_m" | "cashback" | "cashback_fijo">("descuento_pct");
  ...
  const clientesFiltrados = clientes.filter((c) => {
    ...
    const telDigitos = (c.telefono ?? "").replace(/\D/g, "");
    const rucDigitos = (c.ruc ?? "").replace(/\D/g, "");
    return c.nombre.toLowerCase().includes(q)
      || (c.ruc ?? "").toLowerCase().includes(q)
      || (c.telefono ?? "").toLowerCase().includes(q)
      || (qDigitos.length > 0 && (telDigitos.includes(qDigitos) || rucDigitos.includes(qDigitos)));
  }).slice(0, 50);
        ...
        <div className="space-y-3 pb-2">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {(["descuento_pct","descuento_fijo","lleve_n_pague_m","cashback","cashback_fijo"] as const).map((t) => (
            ...
            <div>
              ...
              {clienteSel ? (
                <div className="flex items-center justify-between rounded-lg border border-[#4FAEB2] bg-[#4FAEB2]/5 px-3 py-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] text-slate-500">
                      {clienteSel.ruc && <>RUC {clienteSel.ruc}</>}
                      {clienteSel.ruc && clienteSel.telefono && <> · </>}
                      {clienteSel.telefono && <>Tel {clienteSel.telefono}</>}
                    </p>
                  </div>
                  <button type="button" onClick={() => { setClienteSel(null); setClienteQuery(""); setClientePickerOpen(false); }}
                ...
                <div className="relative">
                  ...
                  <button
                    type="button"
                    onClick={() => setClientePickerOpen((v) => !v)}
                    className="w-full flex items-center justify-between gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-left text-sm hover:border-[#4FAEB2] focus:outline-none focus:ring-2 focus:ring-[#4FAEB2]"
                  >
                    <span className="text-slate-400">
                      {clientesLoading ? "Cargando clientes…" : "Elegir cliente…"}
                    </span>
                    <svg viewBox="0 0 20 20" fill="currentColor" className={`h-4 w-4 text-slate-400 transition ${clientePickerOpen ? "rotate-180" : ""}`}>
                    ...
                    <div className="absolute z-20 mt-1 w-full rounded-lg border border-slate-200 bg-white shadow-lg">
                      <div className="p-2 border-b border-slate-100">
                        <input
                          ...
                          className="w-full rounded-md border border-slate-200 bg-slate-50 px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-[#4FAEB2] focus:bg-white"
                        />
                      </div>
                        ...
                        {clientesFiltrados.length === 0 ? (
                          ...
                          clientesFiltrados.map((c) => (
                            <button
                              key={c.id}
                              type="button"
                              onClick={() => { setClienteSel(c); setClienteQuery(""); setClientePickerOpen(false); }}
                            ...
                            >
                              ...
                              <span className="text-[11px] text-slate-500 shrink-0 tabular-nums text-right">
