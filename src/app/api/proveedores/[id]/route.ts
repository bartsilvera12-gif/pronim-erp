...
/**
 * DELETE /api/proveedores/[id] — borra el proveedor. Si está referenciado por
 * compras u otros registros (FK 23503) lo desactiva (estado=inactivo) para no
 * romper el historial. `mode`: "hard" (borrado) | "soft" (desactivado).
 */
export async function DELETE(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    try {
      ...
      return NextResponse.json(successResponse({ id, mode: "hard" }));
    } catch (e) {
      const code = (e as { code?: string })?.code;
      if (code === "23503") {
        // Referenciado por compras/movimientos → soft-delete.
        await updateProveedor(schema, empresaId, id, { estado: "inactivo" });
        return NextResponse.json(successResponse({ id, mode: "soft" }));
      }
      throw e;
    }
  } catch (err) {
