...
export async function POST(req: Request) {
  try {
    ...
    // Cupo del plan: era un límite legacy de 4 usuarios activos por empresa
    // (Zentra SaaS). Pronim es self-hosted y no tiene ese constraint —
    // Karen tiene multiples cajeras + admins + operadoras BR / PY.
    // Removido: si un cliente futuro necesita el cupo, se reintroduce por
    // env var LIMITE_USUARIOS o config en empresas.
    // (Antes: LIMITE_USUARIOS_EMPRESA = 4 bloqueaba el 5to alta.)
