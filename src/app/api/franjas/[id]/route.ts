 ...
 * Body: { precio_venta?: number, activo?: boolean }
 *
 * Solo super_admin. Al cambiar el precio, actualiza nombre y sku
 * para mantener consistencia (Prenda Gs. X / FRJ-X).
 */
...
export async function PATCH(
  ...
  try {
    ...
    // UNIQUE constraint (empresa_id, sku) — precio duplicado
    if (msg.includes("productos_empresa") || msg.includes("unique") || msg.includes("duplicate")) {
      return NextResponse.json(
        errorResponse("Ya existe una franja con ese precio."),
        { status: 409 },
      );
    }
