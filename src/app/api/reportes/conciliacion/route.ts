...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getReporteConciliacion(schema, empresaId, { mes, start, end, mesInicio: `${mes}-01` });
    return NextResponse.json(successResponse(data));
  } catch (err) {
    console.error("[/api/reportes/conciliacion]", err instanceof Error ? err.message : err);
    return NextResponse.json(errorResponse("No se pudo cargar la conciliación."), { status: 500 });
  }
}
